...
public class SimulationSetup {
    ...
    private static final Logger logger = Logger.getLogger(SimulationSetup.class);

    /**
     * Lists the possible reasons why a SimulationSetup can be changed and
     * unsaved
     */
    public enum CHANGED {
     ...
     */
    @XmlTransient
    private HashMap<String, DefaultListModel> hashMap4AgentDefaulListModels = new HashMap<>();
    /**
     * The ComboBoxModel for agent lists.
     */
    @XmlTransient
    private DefaultComboBoxModel comboBoxModel4AgentLists = new DefaultComboBoxModel();
    /**
     * The agent list to save.
     */
    @XmlElementWrapper(name = "agentSetup")
    @XmlElement(name = "agent")
    private ArrayList<AgentClassElement4SimStart> agentList = new ArrayList<>();
    /**
    ...
    /**
     * This field can be used in order to provide customised objects during the
     * runtime of a project. This will be not stored within the file
     * 'agentgui.xml'
     */
    @XmlTransient
    ...
    /**
     * Constructor without arguments (This is first of all for the JAXB-Context
     * and should not be used by any other context).
     */
    ...
    private void mergeListModels() {
        ...
        Vector<String> agentListNames = new Vector<>();
    ...
    /**
     * This Method transfers a DefaultListModel to the localArrayList
     * 'agentList' which is a type of 'AgentClassElement4SimStart'.
     *
     * @param lm the DefaultListModel that has to be added to the overall agent
     * list
     */
    ...
    /**
     * This method will create all DefaultListModels which will be used within
     * the visible application<br> as for example for the manual agent
     * configuration (tab 'Simulation-Setup' => 'Agent-Start')<br> or the
     * configuration of the environment model (tab 'Simulation-Setup' =>
     * 'Simulation Environment').<br> The resulting ListModels can be get by
     * using {@link #getAgentDefaultListModel(String)}
     */
    public void createHashMap4AgentDefaulListModelsFromAgentList() {

        if (this.hashMap4AgentDefaulListModels == null) {
            this.hashMap4AgentDefaulListModels = new HashMap<>();
        }
    ...
    /**
     * Here a complete agent start list (DefaultListModel) can be added to the
     * simulation setup.
     *
    ...
    /**
     * This method can be used in order to add an individual agent start list to
     * the SimulationSetup.<br> The list will be filled with elements of the
     * type {@link AgentClassElement4SimStart} coming from the stored setup file
     * and will be later on also stored in the file of the simulation setup.
     *
     * @see AgentClassElement4SimStart AgentClassElement4SimStart - The type to
     * use within a concrete list model
     *
     * @param newDefaultListModel4AgentStarts the new DefaultListModel to set
     * @param listName the name of the list to be assigned. Consider the use of
     * one of the constants {@link #AGENT_LIST_ManualConfiguration} or
     * {@link #AGENT_LIST_EnvironmentConfiguration} or just use an individual
     * name
     * @return the DefaultListModel of agents to be started for the specified
     * list
     */
    ...
    /**
     * Checks if an agent name already exists in the current agent
     * configuration.
     *
     ...
     * @param mergeListModels indicates if the over all {@link #agentList} has
     * to be build new
     * @return true, if the agent name already exists
     */
    ...
    /**
     * Will find a new unique name for an agent, if the suggestion is not
     * already unique.
     *
    ...
    public HashMap<String, String> getTimeModelSettings() {
        if (this.timeModelSettings == null) {
            this.timeModelSettings = new HashMap<>();
        }
