...
import org.gaia.simulationService.agents.ServerMasterAgent;
...
public class LoadInformation {
     ...
     */
    public Vector<String> containerQueue = new Vector<>();
    /**
     * The Hashtable containerDescription, which holds the information
     * identifiable by the container name.
     */
    public HashMap<String, NodeDescription> containerDescription = new HashMap<>();
    /**
     * The Hashtable containerLoads, which holds the load information
     * identifiable by the container name.
     */
    public HashMap<String, PlatformLoad> containerLoads = new HashMap<>();
    /**
     * The Hashtable containerLocations, which holds the location identifiable
     * by the container name.
     */
    public HashMap<String, Location> containerLocations = new HashMap<>();
    /**
     * The agents locations (answers to: where can I find a specified agent).
     */
    public LoadAgentMap agentLocations = new LoadAgentMap();
    /**
     * The Vector of agents, which are connected by the actuator/sensor
     * relationship of the SimulationService.
     */
    public Vector<AID> sensorAgents = null;
    /**
     * The Hashtable, which holds the list of remote container, that should join
     * the current platform.
     */
    private HashMap<String, Container2Wait4> newContainers2Wait4 = new HashMap<>();
    private long cycleTimeStart = 0;
    private Vector<Long> cycleTimeVector = new Vector<>();
    ...
    /**
     * Method to get the current cycle-frequency in average. The average will be
     * calculated out of 500 single values in maximum. If a further cycle time
     * occurs, the first measurment will be removed and so on.
     */
    public double getAvgCycleTime() {

        if (this.cycleTimeVector.isEmpty()) {
            return 0;
        }
    ...
    /**
     * Method to put the answer of the Server.Master directly in the local
     * public containerDescription - Variable
     *
     * @param crcReply the ClientRemoteContainerReply, which describes a single
     * container and its machine.
     */
    public void putContainerDescription(ClientRemoteContainerReply crcReply) {
        ...
        if (!containerQueue.contains(node.getContainerName())) {
            containerQueue.addElement(node.getContainerName());
        }
    }

    /**
     * Sets the container name on which the running system is waiting for,
     * because a new remote container was requested.
     *
    ...
    /**
     * Returns the status of the container on which the system is waiting for in
     * order to join the current platform.
     *
     * @param containerName the container name
     * @return the status of the new container described by the class
     * Container2Wait4
     *
    ...
    /**
     * This method sets that the new RemoteContainerRequest was NOT
     * successfully.
     *
    ...
    /**
     * Resets the Object, which holds the Informations about the agent in
     * relation to it's locations etc.
     *
     * @see #agentLocations
     */
    ...
    /**
     * Method to put the Agent-AIDs which are coming from the currently running
     * containers.
     *
     * @param containerName the container name
    ...
    public void putAIDs4Container(String containerName, AID[] aid) {
        ...
        if (aid.length == 0) {
            ...
            for (int i = 0; i < aid.length; i++) {
                ...
                if (sensorAgents.contains(aid[i])) {
                    hasSensor = true;
                } else {
                    hasSensor = false;
                }
                if (aid[i].getLocalName().startsWith("Gaia")) {
    ...
    /**
     * This method starts the counting for the agents at the platform and the
     * agents, which are located at one specific container
     */
    ...
    // --------------------------------------------------------------
    /**
     * This class describes the container(computer), which are connected to the
     * Main-Container. There are informations about CPU, Memory and operating
     * system available as well as the value from benchmark-test
     */
    public class NodeDescription {
        ...
        /**
         * Constructor of this Sub-Class, using the ClientRemoteContainerReply
         * from the {@link ServerMasterAgent}.
         *
